...
const QUICK_PROMPTS: Record<BusinessType, string[]> = {
  ...
  hvac: ["My AC stopped working", "Can you service my furnace?", "What's a maintenance plan cost?"],
  auto: ["My check engine light is on", "I need an oil change", "Can you do brake repair?"],
  law: [
...
export function LightTextDemo() {
  ...
  const rotateX = useSpring(useTransform(mouseY, [-200, 200], [3, -3]), tiltSpringConfig);
  const rotateY = useSpring(useTransform(mouseX, [-300, 300], [-3, 3]), tiltSpringConfig);
  ...
  const { conversationHistory, isLoading, error, isTyping, currentAiResponse } = state;
