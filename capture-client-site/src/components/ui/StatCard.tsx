...
export function StatCard({
  ...
  if (variant === "minimal") {
    return (
      ...
      >
        {/* Icon */}
        {icon && <div className={cn("w-10 h-10", colorClasses[color])}>{icon}</div>}

        {/* Value */}
        <div className="flex items-baseline gap-1">
          <span className="text-sm text-slate-600">{prefix}</span>
          <motion.span
            className={cn("font-display text-4xl md:text-5xl font-bold", colorClasses[color])}
          >
            {display}
          </motion.span>
    ...
    >
      ...
      >
        <div className="p-6 space-y-4">
          {/* Icon */}
          {icon && <div className={cn("w-12 h-12", colorClasses[color])}>{icon}</div>}
